...
for i in range(650):
    ...
    torch.save(torch.FloatTensor(fuel[81:]).clone(),"/root/autodl-tmp/data/fuel_sample" + str(i) + ".pt")
    windu=(np.array(out[i].zarr['windu']))
    windv=(np.array(out[i].zarr['windv']))
    torch.save(torch.FloatTensor(windv[81:]).clone(),"/root/autodl-tmp/data/windv_sample" + str(i) + ".pt")
    torch.save(torch.FloatTensor(windu[81:]).clone(),"/root/autodl-tmp/data/windu_sample" + str(i) + ".pt")
